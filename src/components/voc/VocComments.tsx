...
import type { VocComment, Profile } from '../../types';
...
export default function VocComments({ vocId }: VocCommentsProps) {
  const { user } = useAuthStore();
  const { addToast } = useUiStore();
  const { fetchVocComments, addVocComment, deleteVocComment } = useVocs();
  ...
  const loadComments = async () => {
    setLoading(true);
    const result = await fetchVocComments(vocId);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const { data, error } = await addVocComment(vocId, user.id, input.trim());
    setSubmitting(false);
    if (error) {
  ...
  const handleDelete = async (commentId: string) => {
    const { error } = await deleteVocComment(commentId);
    if (error) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <h3 className="flex items-center gap-1.5 text-xs font-semibold text-text-muted">
        ...
        공감 댓글 {comments.length > 0 && `(${comments.length})`}
      </h3>
      ...
      <div ref={listRef} className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
        {loading ? (
          ...
          <p className="text-xs text-text-muted py-3 text-center">아직 댓글이 없습니다. 공감이나 의견을 남겨보세요!</p>
        ) : (
          comments.map((c) => (
