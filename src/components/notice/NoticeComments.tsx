...
export default function NoticeComments({ noticeId, fetchComments, addComment, deleteComment }: NoticeCommentsProps) {
  ...
  const displayName = (authorId: string) => {
    const p = profiles.get(authorId);
    return p ? (p.nickname || p.name) : '알 수 없음';
  };

  const authorEmoji = (authorId: string) => profiles.get(authorId)?.avatar_emoji ?? '👤';
  const authorColor = (authorId: string) => profiles.get(authorId)?.avatar_color ?? '#666';
