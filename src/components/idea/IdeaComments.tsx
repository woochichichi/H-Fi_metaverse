...
import type { IdeaComment, Profile } from '../../types';
...
export default function IdeaComments({ ideaId }: IdeaCommentsProps) {
  ...
  const { fetchIdeaComments, addIdeaComment, deleteIdeaComment } = useIdeas();
  ...
  const loadComments = async () => {
    setLoading(true);
    const result = await fetchIdeaComments(ideaId);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const { data, error } = await addIdeaComment(ideaId, user.id, input.trim());
    setSubmitting(false);
    if (error) {
  ...
  const handleDelete = async (commentId: string) => {
    const { error } = await deleteIdeaComment(commentId);
    if (error) {
  ...
  return (
    <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-white/[.06]">
      ...
      {/* 댓글 목록 */}
      <div ref={listRef} className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
        {loading ? (
          <div className="flex gap-2 rounded-lg bg-white/[.03] p-2 animate-pulse">
          ...
          <p className="text-[11px] text-text-muted py-2 text-center">의견을 남겨보세요!</p>
        ) : (
          comments.map((c) => (
            <div key={c.id} className="group flex gap-2 rounded-lg bg-white/[.04] p-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-medium text-text-primary">{displayName(c.author_id)}</span>
                ...
                <p className="text-[11px] text-text-secondary whitespace-pre-wrap break-words leading-relaxed mt-0.5">
                  {c.content}
                </p>
              </div>
            </div>
      ...
      {user && (
        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          <input
            ...
            maxLength={500}
            className="flex-1 rounded-lg bg-white/[.06] px-3 py-1.5 text-[11px] text-text-primary placeholder-text-muted outline-none transition-colors focus:bg-white/[.1]"
          />
          <button
            onClick={(e) => { e.stopPropagation(); handleSubmit(); }}
            disabled={submitting || !input.trim()}
            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-accent text-white transition-colors hover:bg-accent/80 disabled:opacity-40"
          >
            <Send size={12} />
          </button>
        </div>
